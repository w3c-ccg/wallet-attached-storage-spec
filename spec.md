...
* [=encryption-immutable=] (409) -- the update tried to change the `scheme`,
  ...
  [[[#collection-metadata-data-model]]]). Also raised when the update changes
  the `id` or `type` of the descriptor's `hmac` member, or removes that member;
  the blinding key is permanent once present (see [[[#blinding-key-member]]]).
* [=invalid-request-body=] (400) -- the request body is not a JSON object; the
  `custom` object (or a property within it) does not have the shape described in
  [[[#collection-metadata-data-model]]]; a top-level `epoch` member is present
  but is not a non-empty string; the descriptor's key-epoch or `hmac` members
  are malformed, or the update violates a server-side invariant on the epoch
  members (`epochs` append-only, `currentEpoch` never moving backwards, see
  [[[#key-epochs]]]); the resulting object would carry both `plaintext` and
  `encryption` (whichever of the two the update adds); or the supplied
  `plaintext` member is malformed.
...
segment under the Collection URL; see [[[#collection-metadata-data-model]]].)
...
caller already authorized to write; see [[[#encryption-scheme-registry]]]):
